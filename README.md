...
5. In the **.env** file make the following changes:
  ...
  - Optionally, add a **TUNNEL_SUBDOMAIN**. This should be at least three, unique words separated by hyphens if in use (can leave blank to have one autogenerated). This will ensure that the same link is used between tunnel connections instead of generating new links each time (and requiring an update on your Dev App settings each time)
